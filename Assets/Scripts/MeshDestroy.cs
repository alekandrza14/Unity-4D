...
public class MeshDestroy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        selfShadow = Instantiate(gameObject,transform.position, transform.rotation);
        if (selfShadow.GetComponent<MultyObject>())
        {
            selfShadow.GetComponent<MultyObject>().enabled = false;
            selfShadow.GetComponent<MultyObject>().shape = Shape.shape3D;
        }
        selfShadow.GetComponent<MeshDestroy>().enabled = false;
        if (selfShadow.GetComponent<MeshRenderer>())
        {
            selfShadow.GetComponent<MeshRenderer>().material = Resources.Load<Material>("glass");
            selfShadow.GetComponent<MeshRenderer>().enabled = true;
        }
        if (selfShadow.GetComponent<Collider>())
        {
            selfShadow.GetComponent<Collider>().enabled = false;
        }
       if(selfShadow.GetComponent<MeshCollider>()) selfShadow.GetComponent<MeshCollider>().enabled = false;
        selfShadow.SetActive(true);
