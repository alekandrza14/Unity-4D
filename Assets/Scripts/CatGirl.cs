...
public class CatGirl : Enemy
{
    ...
    void Start()
    {
      ...
      if(FindObjectOfType<fristPersonControler>())  fpc = FindObjectOfType<fristPersonControler>();
        //   Instantiate(Particles, transform.position, Quaternion.identity);
    ...
    void Update()
    {
        Ray ray = fpc.g[1].GetComponent<Camera>().ScreenPointToRay(Input.mousePosition);
        RaycastHit hit;
        if (Physics.Raycast(ray,out hit))
        {
            if (hit.collider.gameObject==gameObject)
            {
                fristPersonControler.interes *= 0.2f;
                fristPersonControler.interes -= 10f;
            }
        }
        if (player)
        {
